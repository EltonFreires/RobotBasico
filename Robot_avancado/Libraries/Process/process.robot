...
Task Tags       ex_process

Suite Teardown    Teardown suite process

*** Variables ***
${PATH_PROCESS}        ./Robot_avancado/Libraries/Process
...
Execute um script e aguarde ele finalizar com sucesso
    ...
    ${MEU_PROCESSO}     Run Process     python   ${PATH_PROCESS}/hello_world.py
...
Execute um script e aguarde ele finalizar com falha
    ## Inicia o processo e espera pelo seu término
    ${MEU_PROCESSO}     Run Process     python   ${PATH_PROCESS}/hello_world_fail.py
    Log    Resultado da execução (sucesso/falha): ${MEU_PROCESSO.rc}
    Log    Saída da execução: ${MEU_PROCESSO.stdout}
...
Executando comandos diversos de prompt
    ...
    ## OBS: Após esse passo, criei o teardown suite para deletar o diretório
    Run Process    mkdir robot_teste   cwd=${CURDIR}   shell=True

Teardown suite process
    Remove Directory    ./Robot_avancado/Libraries/Process/robot_teste
